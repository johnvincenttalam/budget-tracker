...
export default function App() {
  ...
  return (
    <div className="bg-slate-950 text-white min-h-dvh max-w-2xl mx-auto relative overflow-hidden theme-transition">
      {/* Lock screen */}
      {isLocked && pinHash && (
      ...
      {/* Top bar */}
      {showTopBar && (
        <div className="sticky top-0 z-30 bg-slate-950/95 backdrop-blur-xl border-b border-slate-800/30">
          <div className="max-w-2xl mx-auto flex items-center justify-between px-5 py-3.5">
            ...
            <h1 className="text-[15px] font-semibold text-white">{screenTitles[screen]}</h1>
            <div className="w-9" />
          </div>
        </div>
      )}
      ...
      {/* Screen content */}
      <div key={screen + (editTransactionId ?? '')} className={`animate-fade-in ${showBottomNav ? 'pb-[6.5rem]' : ''}`}>
      ...
      {showBottomNav && (
        <>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50" style={{ paddingBottom: 'env(safe-area-inset-bottom, 0px)' }}>
            <div className="max-w-2xl mx-auto bg-slate-900/95 backdrop-blur-xl border-t border-slate-800/50 px-6 py-3 grid grid-cols-5 gap-6 items-center">
              {/* Home */}
              <button
                onClick={() => navigate('dashboard')}
                className={`flex flex-col items-center gap-1 ${screen === 'dashboard' ? 'text-emerald-400' : 'text-slate-400'}`}
              >
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <button
                ...
                className={`flex flex-col items-center gap-1 ${screen === 'bills' ? 'text-emerald-400' : 'text-slate-400'}`}
              >
              ...
              {/* FAB */}
              <button
                onClick={() => { haptic(); setFabOpen(!fabOpen); }}
                className={`-mt-8 mx-auto w-14 h-14 rounded-full flex items-center justify-center transition-all duration-200 active:scale-95 shadow-lg shadow-emerald-500/25 ${
                  fabOpen
