        ...
        <div className="px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-slate-200">Expected salary</p>
                <p className="text-[10px] text-slate-500">Shown on payday countdown</p>
              </div>
            </div>
            {editingSalary ? null : store.expectedSalary > 0 ? (
              <div className="flex items-center gap-1.5">
                <button
                  onClick={() => { setEditingSalary(true); setSalaryInput(String(store.expectedSalary)); }}
                  className="text-xs text-emerald-400 bg-slate-800 px-2 py-1 rounded-lg"
                >
                  {formatMoney(store.expectedSalary, sym)}
                </button>
                <button onClick={() => store.setExpectedSalary(0)} className="text-slate-600 text-xs">×</button>
              </div>
            ) : (
              <button
                onClick={() => { setEditingSalary(true); setSalaryInput(''); }}
            ...
            <div className="flex gap-2 mt-2">
              ...
              <button
                onClick={() => {
                  ...
                  store.setExpectedSalary(val > 0 ? val : 0);
